...
export default function NovaRespostaRapidaPage() {
  ...
  /* normaliza o atalho enquanto digita */
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!r.ok) { const d = await r.json(); setError(d.error || 'Erro ao criar resposta.'); return }
    setSuccess(true)
      ...
      <div className={s.container} style={{ alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div style={{
          ...
          <p style={{ color: '#64748b', margin: 0 }}>
            Use <code style={{ background: '#ede9fe', color: '#6d28d9', borderRadius: 6, padding: '2px 8px', fontWeight: 700 }}>/{name}</code> no chat para inserir. Redirecionando...
          </p>
        </div>
      </div>
    )
    ...
    <div className={s.container}>
      ...
      <div className={s.pageHeader}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h1 className={s.pageTitle}>Nova resposta rápida</h1>
            <p className={s.pageSubtitle}>Crie um atalho para mensagens frequentes</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 24 }}>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: '0.8125rem', color: '#94a3b8' }}>
                  → será usado no chat digitando <strong style={{ color: '#6d28d9' }}>/{name || 'atalho'}</strong>
                </span>
              </div>
          ...
          <div style={{
            ...
            <div>
              <p style={{ margin: '0 0 4px', fontWeight: 700, fontSize: '0.875rem', color: '#5b21b6' }}>
                Como usar no chat
              </p>
              <p style={{ margin: 0, fontSize: '0.8125rem', color: '#7c3aed', lineHeight: 1.55 }}>
                Durante um atendimento, digite <code style={{ background: '#ddd6fe', borderRadius: 4, padding: '1px 6px', fontWeight: 700 }}>/</code> seguido do nome do atalho no campo de mensagem. Um menu aparecerá com as opções disponíveis. Use as setas para navegar e <strong>Enter</strong> ou <strong>Tab</strong> para inserir.
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
            ...
            >
              {saving
                ? <><Loader2 size={16} style={{ animation: 'spin 0.8s linear infinite' }} /> Criando...</>
                : <><Save size={16} /> Criar resposta rápida</>
              }
            </button>
          </div>
