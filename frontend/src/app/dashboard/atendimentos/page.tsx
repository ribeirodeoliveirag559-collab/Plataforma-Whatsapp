...
function ChatPanel({
  ...
  const [activeTab, setActiveTab]       = useState<'chat' | 'resumo'>('chat')
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex border-b border-slate-200 shrink-0 bg-white">
        ...
        {(ticket.aiSummary || ticket.contact.company) && (
          <button
          ...
          >
            <Bot size={13} /> Resumo IA
            {activeTab !== 'resumo' && ticket.aiSummary && (
      ...
      {activeTab === 'resumo' && (
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-indigo-50 to-violet-50 px-5 py-3 border-b border-slate-100 flex items-center gap-2">
              <Bot size={15} className="text-indigo-600" />
              <span className="text-sm font-semibold text-indigo-700">Resumo coletado pela IA</span>
            </div>
            <div className="divide-y divide-slate-100">
              <div className="px-5 py-3 flex items-start gap-3">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide w-24 shrink-0 pt-0.5">Cliente</span>
                <span className="text-sm text-slate-800 font-medium">{ticket.contact.name || '—'}</span>
              </div>
              <div className="px-5 py-3 flex items-start gap-3">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide w-24 shrink-0 pt-0.5">Telefone</span>
                <span className="text-sm text-slate-700">{ticket.contact.number}</span>
              </div>
              {ticket.contact.company && (
                <div className="px-5 py-3 flex items-start gap-3">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide w-24 shrink-0 pt-0.5">CNPJ / Empresa</span>
                  <span className="text-sm text-slate-700">{ticket.contact.company}</span>
                </div>
              )}
              {ticket.queue && (
                <div className="px-5 py-3 flex items-start gap-3">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide w-24 shrink-0 pt-0.5">Departamento</span>
              ...
              {ticket.aiSummary && (
                <div className="px-5 py-3 flex items-start gap-3">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide w-24 shrink-0 pt-0.5">Demanda</span>
                  <p className="text-sm text-slate-700 leading-relaxed">{ticket.aiSummary}</p>
                </div>
              )}
            </div>
          </div>
